    ...
    <div className="pt-32 pb-24">
      <section className="relative">
        ...
        <div className="container-x relative max-w-3xl mx-auto">
          ...
          <p className="mt-4 text-[11px] font-bold uppercase tracking-[0.24em] text-cream/50">
            {t("legal.updated")}: {t("legal.date")}
          </p>
          <p className="mt-8 text-cream/75 leading-relaxed">{doc.intro}</p>
            ...
            {doc.sections.map((s, i) => (
              <section key={i}>
                ...
                {s.paragraphs?.map((p, j) => (
                  <p key={j} className="text-cream/75 leading-relaxed mb-3">
                    {p}
                  </p>
                ))}
                {s.bullets && (
                  <ul className="mt-2 space-y-2 list-disc pl-5 text-cream/75 leading-relaxed">
                    {s.bullets.map((b, j) => (
                      <li key={j}>{b}</li>
                    ))}
                  </ul>
